...
======================================================================
Portierte, fuer die App-Nutzung leicht angepasste Version von
shc_resource_reader.py (siehe dort fuer die vollstaendige Herleitungs-
Historie der einzelnen Adressen). Die Kern-Lesefunktionen sind unveraendert
- nur write_overlay_json/write_overlay_file wurden in eine reine
Payload-Bau-Funktion (build_overlay_payload) plus duenne Datei-Wrapper
...
RESOURCE_OFFSETS = {
    ...
    "unit_archer":      0x1BE8,  # Bogenschuetze
    "unit_crossbowman": 0x1BEC,  # Armbrustschuetze
    "unit_spearman":    0x1BF0,  # Speertraeger
    "unit_pikeman":     0x1BF4,  # Pikenier
    "unit_maceman":     0x1BF8,  # Streitkolbenkaempfer
    "unit_swordsman":   0x1BFC,  # Schwertkaempfer
    "unit_knight":      0x1C00,  # Ritter
    "unit_laddercarrier": 0x1C04,  # Leitertraeger
    "unit_tunneler":    0x1C0C,  # Tunnelbauer
    "unit_builder":     0x1C08,  # Baumeister

    "unit_arab_archer":    0x2558,  # Arabischer Bogenschuetze
    "unit_horse_archer":   0x255C,  # Berittener Bogenschuetze
    "unit_slave":          0x2560,  # Sklave
    "unit_slinger":        0x2568,  # Schleuderschuetze
    "unit_fire_thrower":   0x256C,  # Feuerwerfer
    "unit_arab_swordsman": 0x2570,  # Arabischer Schwertkaempfer
    "unit_assassin":       0x2564,  # Assassine
}

# Popularity (Gesamtbeliebtheit) liegt VOR dem fruit_basket-Feld, Rohwert
# 10000 = 100%. Live per Increased-value-Scan ueber einen Monatswechsel
# gefunden und bestaetigt (2026-09-01): der urspruengliche Kandidat -0x40C
# war falsch (zeigte in leeren Slots faelschlich Speichermuell als 1-3% an);
# der neue Wert -0x470 stimmte exakt mit der Ingame-Anzeige ueberein (97%->
# 98% nach einem Monat mit ausschliesslich "Steuern +1" als aktivem Faktor).
POPULARITY_OFFSET = -0x470

# Bevoelkerung (aktuelle Einwohner) und die Kapazitaet (steigt um +8 pro
# gebautem Haus, Start bei 10) - im Spiel als "x/y" im Bevoelkerungsbuch
# angezeigt. Live per Speicher-Scan gefunden und bestaetigt (2/10 -> 5/18
# nach einem neuen Haus, exakt getroffen). Bei leeren Slots beide 0.
POPULATION_OFFSET = -0x458
POPULATION_CAPACITY_OFFSET = -0x45C

# Steuersatz als 0-indizierter Skalen-Wert (0 = "Grosszuegige Spende" / +7
...
# nach unten bestaetigt. Ueber alle 8 Slots verifiziert: KI-Slots liegen
# alle auf 3 (Standard "Keine Steuern"), der Mensch-Slot passend zum
# tatsaechlich eingestellten Wert - zwei andere Kandidaten-Adressen zeigten
# bei KI-Slots ueberall 0 (kein gueltiger Steuersatz) und wurden verworfen.
TAX_RATE_OFFSET = 0x1CB8

# Live-HP des Burgherren pro Slot - eigene Basis/Schrittweite, siehe
# read_player() fuer die Herleitung.
LORD_HP_BASE = 0x1388DA4
LORD_HP_STRIDE = 0x490

# Max-HP eines Burgherren = LORD_BASE_HP * Multiplikator je nach
# KI-Persoenlichkeit (Feld "lord.StrengthMultiplier" in einem lokalen
...
# volle 16-Lord-Tabelle stammt aus einem Mehrheitskonsens ueber 14
# unabhaengige Custom-AI-Mod-Exporte (jeder Mod ueberschreibt nur seinen
# einen Lord-Slot) - zwei Werte (Ratte, Wolf) zusaetzlich live im Speicher
# bestaetigt (100000*1.5=150000 fuer Wolf, exakt getroffen). Menschliche
# Spieler haben keinen dieser Namen im Roster-String und fallen auf den
# Standard-Multiplikator 1.0 zurueck.
LORD_BASE_HP = 100000
...
def get_lord_max_hp(display_name):
    ...
    Namen und gibt LORD_BASE_HP * dessen Staerke-Multiplikator zurueck.
...
# Team-/Buendnis-Zugehoerigkeit: KEIN Live-Feld gefunden (Stand 2026-09-01),
...
# gemischten KI-Personas um Zufallstreffer auszuschliessen) UND einem
...
# (3+3+2) als Zufall - siehe project_shc_overlay_status.md fuer die volle
# Historie. Team-Zuordnung im Overlay laeuft deshalb ausschliesslich ueber
...
# feste Tabelle im Spiel (aendert sich nie zur Laufzeit) - deshalb per
# manueller Ingame-Ablese-Tabelle hinterlegt statt per Speicher-Scan (ein
# Exact-Value-Scan nach einer Steuerstufen-Aenderung lieferte 0 Treffer, weil
# es eben keine live geschriebene Adresse dafuer gibt). Quelle: User hat alle
# 12 Stufen im Beliebtheits-Aufschluesselungs-Dialog abgelesen (2026-09-01).
TAX_POPULARITY_EFFECT = {
    0: 7,    # Grosszuegige Spende
    1: 5,    # Grosse Spende
    2: 3,    # Kleine Spende
    3: 1,    # Keine Steuern
...
# Sammel-Zaehler fuer BEWEGLICHE Belagerungsgeraete (Schild, Rammbock,
# Belagerungsturm, Katapult, Tribok, Feuerballiste zusammen). Siehe
# shc_resource_reader.py fuer die vollstaendige Herleitung.
...
# Unit-Typen, fuer die es echte Live-Einzel-Offsets gibt (fuer die
# "Truppen-Aufschluesselung"-Anzeige im Overlay) - alle RESOURCE_OFFSETS
# Keys, die mit "unit_" beginnen.
UNIT_TYPE_KEYS = [k for k in RESOURCE_OFFSETS if k.startswith("unit_")]

# Die 4 essbaren Nahrungsmittel, die im Kornspeicher gezaehlt werden (Mehl
# und Hopfen sind Zwischenprodukte fuer Brot/Bier und zaehlen NICHT mit).
# Bier zaehlt bewusst NICHT als Nahrung: es wird nicht von der Bevoelkerung
...
def connect(verbose: bool = True):
    ...
    zurueck - error_message ist None bei Erfolg."""
    try:
        ...
        msg = f"Prozess '{PROCESS_NAME}' nicht gefunden. Laeuft das Spiel?"
        if verbose:
            print(f"[FEHLER] {msg}")
        return None, msg
...
def read_player(pm, player_index):
    """Liest alle bekannten Werte fuer einen Spieler-Slot aus."""
    ...
    # Kampfhandlung) ueberzeugend nach einem festen Pro-Spieler-Array aus -
    # war aber ein Trugschluss: es ist derselbe Ereignis-Ringpuffer wie bei
    # den Moenchen, und die 8 isolierten Treffer landeten nur zufaellig in
    ...
    # KI etc.) zeigte diese Formel fuer ALLE Spieler denselben (veralteten/
    # generischen) Wert - live bestaetigt falsch. Eine echte Umsetzung
    # braucht dieselbe Ereignis-Zaehler-Technik wie poll_monk_events()
    # (Ereignis-Typ fuer "Schaden mit resultierender HP" identifizieren,
    # Bereich durchlaufen), nicht eine feste Adresse. Siehe
    # project_shc_overlay_status.md fuer die volle Historie.
    values["lord_hp"] = None
...
# gefundene Weg, Moenche pro Spieler zu zaehlen (es gibt KEIN festes
# Zaehlerfeld dafuer im Spieler-Struct, siehe Projekt-Notizen: sehr
# gruendlich mit 5+ unabhaengigen Methoden gesucht). Reverse engineered per
...
# CrusaderData. Live-verifiziert ueber mehrere Spieler (1,3,4) mit sauber
# isolierten Einzel-Bauten: Typ-ID 101 = Moench, jedesmal exakt und korrekt
# dem bauenden Spieler zugeordnet. Belagerungsgeraete (Tribok/Rammbock/
# Katapult/Feuerballiste/sogar Turm-Balliste) zeigen hier alle dieselbe
# geteilte Typ-ID 6 - keine Einzelaufschluesselung moeglich, auch hier
# nicht (bestaetigt ueber 5 verschiedene Typen).
...
# Modul-weiter State (persistiert ueber mehrere poll_monk_events()-Aufrufe
# hinweg) - noetig, weil das Log ein Ringpuffer ist: wir muessen bei JEDEM
# Poll die seit dem letzten Mal neu hinzugekommenen Eintraege ansehen,
# sonst gehen Ereignisse verloren, sobald der Puffer sie ueberschreibt.
...
def poll_monk_events(pm):
    ...
    damit keine Ringpuffer-Eintraege verloren gehen. Zaehlt neue Moenche
    (Typ-ID 101) seit dem letzten Aufruf pro Spielernummer (1-8) mit.
    Der erste Aufruf nach Tool-Start setzt nur die Baseline (die
    Vergangenheit im Ringpuffer koennte laengst ueberschriebene, nicht mehr
    gueltige Eintraege enthalten - deshalb wird ab Tool-Start gezaehlt,
    nicht rueckwirkend)."""
    ...
    # WICHTIG: der Zaehler zeigt immer den NAECHSTEN NOCH LEEREN Slot (noch
    # nicht geschrieben, liest sich als Spieler=0/Typ-ID=0) - Eintraege
    # 0..counter-1 sind bereits gueltig geschrieben. Der alte gemerkte
    # Zaehler-Wert selbst ist also schon der erste NEUE Index, nicht der
    # letzte alte (off-by-one, live per Vollprotokoll-Vergleich gefunden:
    # ohne dieses "ohne +1" wurde der erste Moench nach jedem Checkpoint
    # verschluckt).
...
def get_monks_trained(player_index):
    ...
    seit Tool-Start trainierter Moenche zurueck (kein Live-Bestand - stirbt
    ein Moench, sinkt dieser Wert NICHT, siehe Docstring von
    poll_monk_events)."""
...
def is_active_slot(values):
    ...
    wurde. Population_capacity ist dafuer der zuverlaessigste Indikator: jeder
    echte Spieler-Slot startet mit Kapazitaet >=10, waehrend ein wirklich
    leerer Slot immer exakt 0 zeigt. Andere Felder (Gold, Truppen, sogar
    popularity_percent) koennen in ungenutzten Slots Speichermuell von einem
    frueheren, groesseren Match enthalten und sind daher NICHT zuverlaessig
...
def read_all_players(pm):
    """Liest alle Slots in PLAYER_INDEX_RANGE, gibt Liste von (index, values) zurueck."""
...
def read_roster_names(pm):
    """Liest die aktuellen Anzeige-Namen live ueber den globalen Roster-
    Zeiger. Gibt dict slot->name zurueck; leer bei Fehlern (z.B. noch keine
    Karte geladen), dann faellt die Anzeige auf slot_label() zurueck."""
    try:
...
def build_overlay_payload(all_values, lord_labels, attack_status=None, display=None):
    """Baut die JSON-Payload fuers Overlay - reine Funktion, kein Datei-I/O.
    ...
    display: optionales Config-"display"-dict, um zu steuern welche Felder
             ...
             der Parameter ist hier fuer zukuenftige serverseitige Filterung
...
# --- Nur fuer CLI-Debug (python app/reader.py), nicht von der App genutzt ---
